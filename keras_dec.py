...
from tensorflow.keras.layers import Layer, InputSpec # W
...
class DeepEmbeddingClustering(object):
    def __init__(self,
        ...
        self.decoder = Sequential(self.decoders) # aggiunto da me TODO
        ...
        self.filenameautoencoder = self.path_base+'/pretrain/'+dev_name + '_autoencoder.h5'
    

        # create folder if not exists
        if not os.path.exists(self.path_base+'/pretrain/'):
            os.makedirs(self.path_base+'/pretrain/')

        if overwrite_pretrain:
            if os.path.isfile(self.filenameautoencoder):
                os.remove(self.filenameautoencoder)
                
        # if file exists
        if os.path.isfile(self.filenameautoencoder):
            self.autoencoder.load_weights(self.filenameautoencoder)
    ...
    def initialize(self, X, verbose=0, save_autoencoder=False, layerwise_pretrain_iters=50000, finetune_iters=100000):
        if not os.path.isfile(self.filenameautoencoder):
            ...
            for i, autoencoder in enumerate(self.layer_wise_autoencoders):
                ...
                autoencoder.fit(current_input, current_input, 
                                batch_size=self.batch_size, verbose=verbose, epochs=layerwise_epochs, callbacks=[lr_schedule, tf.keras.callbacks.EarlyStopping(monitor='loss', patience=3)])
            ...
            self.autoencoder.fit(X, X, batch_size=self.batch_size, verbose=verbose, epochs=finetune_epochs, callbacks=[lr_schedule, tf.keras.callbacks.EarlyStopping(monitor='loss', patience=3)])

            if save_autoencoder:
                self.autoencoder.save_weights(self.filenameautoencoder)
        else:
    ...
    def cluster(self, X, y=None,
        ...
        while train:
            #sys.stdout.write('\r')
            # cutoff iteration
            # if model folder already exists
            finalfile = self.path_base+'/dec/'+ self.dev_name + '_DEC'  
            # if folder not exists, create it
            if not os.path.exists(self.path_base+'/dec/'):
                os.makedirs(self.path_base+'/dec/')

            # if file exists and overwrite false, load it
            if os.path.exists(finalfile) and not overwrite:
                self.DEC = tf.keras.models.load_model(finalfile)
                self.y_pred = self.DEC.predict(X, verbose=0).argmax(1)
                return self.y_pred
            
            if iter_max < iteration:
                print('Reached maximum iteration limit. Stopping training.')
                self.DEC.save(finalfile, save_format='tf')
                return self.y_pred
            ...
            if iteration % update_interval == 0:
                ...
                if y is not None:
                    #acc, w = self.cluster_acc(y, y_pred)[0]
                ...
                # print check delta_label < tol:
                print('DELTA LABEL: ', delta_label, 'TOL: ', tol)
                if delta_label < tol:
            ...
            if iteration % 1000 == -1:
                ...
                # plot points and cluster centres
                plt.scatter(z_2d[:,0], z_2d[:,1], c=self.q.argmax(1), s=0.1, cmap='jet')
                plt.scatter(clust_2d[:,0], clust_2d[:,1], c='black', s=10, alpha=0.5)
                plt.show()
            ...
            if iteration % save_interval == 0:
                ...
                pickle.dump({'z_2d': z_2d, 'clust_2d': clust_2d, 'q': self.q, 'p': self.p},
                            open(finalfile+'_c_'+str(iteration)+'.pkl', 'wb'))
                # save DEC model checkpoints
                self.DEC.save(finalfile+"_iter_"+str(iteration)+'.h5')
